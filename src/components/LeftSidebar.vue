...
async function addImageLayer() {
  ...
  input.onchange = async (e) => {
    ...
    img.onload = () => {
      const layerType = file.type.includes('gif') ? 'gif' as const
        : file.type.includes('svg') ? 'svg' as const : 'image' as const
      const layer = layerManager.createAssetLayer(layerType, {
        id: `asset_${Date.now()}`,
        url: originalUrl,
        mimeType: file.type,
        size: file.size,
...
function addTextLayer() {
  const layer = layerManager.createTextLayer('Double-click to edit', {
    transform: { x: 100, y: 100, width: 300, height: 60, rotation: 0, scaleX: 1, scaleY: 1, opacity: 1 },
  })
  layerManager.addLayer(layer)
}
...
function addShapeLayer() {
  const layer = layerManager.createShapeLayer('rect', {
    transform: { x: 100, y: 100, width: 200, height: 200, rotation: 0, scaleX: 1, scaleY: 1, opacity: 1 },
...
<template>
  <aside class="sidebar" :class="{ collapsed }">
    <button class="toggle-btn" @click="emit('toggle')" :title="collapsed ? 'Expand panel' : 'Collapse panel'">
    ...
    <div class="sidebar-inner">
      ...
      <div class="sidebar-section">
        ...
        <div v-if="sections.find(s => s.id === 'background')?.expanded" class="section-content">
          <div class="grid-options">
            <button class="grid-btn" :class="{ active: bgStore.type === 'color' }" @click="bgStore.setColor('#171717')">Color</button>
            <button class="grid-btn" :class="{ active: bgStore.type === 'image' }" @click="pickBgImage">Image</button>
          </div>
          <div class="prop-row" style="margin-top:8px">
            <label class="prop-label">Color</label>
            <div class="color-input">
              <input type="color" :value="bgStore.color" @input="bgStore.setColor(($event.target as HTMLInputElement).value)" />
        ...
        <div v-if="sections.find(s => s.id === 'ai')?.expanded" class="section-content">
          <div class="ai-card">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M12 2a4 4 0 014 4c0 2-2 3-4 5-2-2-4-3-4-5a4 4 0 014-4z"/><path d="M12 10v4"/><path d="M12 18v.01"/>
            </svg>
...
.toggle-btn {
  ...
  border: 1px solid var(--border-primary); border-right: none;
  border-radius: 4px 0 0 4px; transition: color var(--transition-fast);
}
...
.section-header {
  ...
  letter-spacing: 0.6px; transition: color var(--transition-fast);
}
.section-header:hover { color: var(--text-secondary); }
.section-header svg { transition: transform var(--transition-fast); opacity: 0.5; }
.section-header svg.rotated { transform: rotate(90deg); }
.section-content { padding: 0 var(--space-3) var(--space-3); display: flex; flex-direction: column; gap: var(--space-1); }

.tool-btn {
  display: flex; align-items: center; gap: var(--space-3);
  padding: var(--space-2) var(--space-3); border-radius: var(--radius-md);
  color: var(--text-secondary); font-size: var(--text-sm);
  font-weight: var(--weight-medium); transition: all var(--transition-fast);
  width: 100%; text-align: left;
}
.tool-btn:hover { background: var(--bg-panel-hover); color: var(--text-primary); }
...
.grid-btn {
  ...
  border: 1px solid var(--border-primary); transition: all var(--transition-fast);
}
.grid-btn.active { border-color: var(--accent); color: var(--accent); background: var(--accent-muted); }
.grid-btn:hover { color: var(--text-primary); }
...
.empty-hint { padding: var(--space-4) var(--space-3); color: var(--text-tertiary); font-size: var(--text-sm); font-style: italic; }

.ai-card {
  display: flex; flex-direction: column; align-items: center; gap: var(--space-2);
  padding: var(--space-6) var(--space-4);
  border: 1px dashed var(--border-primary); border-radius: var(--radius-lg);
  color: var(--text-secondary); font-size: var(--text-sm);
  font-weight: var(--weight-medium); text-align: center;
  cursor: pointer; transition: all var(--transition-fast);
}
.ai-card:hover { border-color: var(--accent); background: var(--accent-muted); }
.ai-sub { font-size: var(--text-xs); color: var(--text-tertiary); font-weight: var(--weight-regular); }
</style>
